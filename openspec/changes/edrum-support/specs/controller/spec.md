...
- **AND** its name and kind are shown in the chooser

#### Scenario: Storage is unavailable

- **WHEN** storage is blocked and a controller is configured
- **THEN** the session still plays with that controller
- **AND** no error surfaces to the student

### Requirement: One saved shape for both kinds

A stored controller SHALL record which kind it is, and SHALL describe its pads
uniformly — id, label, role, captured MIDI note and GM sound — whichever kind
it is.

It SHALL additionally write the controller-note and sound-note arrays in the
shape existing readers already consume, derived from the pads on every save, so
any consumer that has not learned the controller object still obtains a working
note map.

A stored configuration with no kind marker SHALL be read as a pad grid exactly
as it is today, with no migration.

#### Scenario: An unaware reader still gets a working map

- **WHEN** a consumer that only understands the legacy shape reads a stored
  controller of either kind
- **THEN** it obtains a controller-note to GM-note map covering every mapped pad

#### Scenario: An existing grid config is untouched

- **WHEN** a configuration saved before this change is loaded
- **THEN** it becomes a pad-grid controller behaving exactly as before
- **AND** nothing rewrites or upgrades it on read

#### Scenario: The pads remain the source of truth

- **WHEN** any pad's note or sound changes and the controller is saved
- **THEN** the mirrored arrays are regenerated from the pads
